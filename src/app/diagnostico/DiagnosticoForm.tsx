...
import { submitDiagnosisAction, addDiagnosisAction } from "@/lib/actions";
...
export function DiagnosticoForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setPending(true);
    setError(null);
    const res = loggedInEmpresa
      ? await addDiagnosisAction({
  ...
  const Topbar = () =>
    loggedInEmpresa ? (
      <AccountTopbar user={{ nombre: userNombre, rol: "empresa" }} />
    ) : (
      <SiteTopbar />
    );
      ...
      <main className="min-h-screen bg-paper">
        ...
        <Container className="flex min-h-[70vh] flex-col items-center justify-center py-24 text-center">
          ...
          <p className="mt-4 max-w-md text-ink/60">
            {loggedInEmpresa
              ? "Vamos a leerlo con atención y coordinar la primera entrevista. Definamos el QUÉ juntos."
              : "Te mandamos un email para confirmar tu cuenta — hacé click en el enlace para poder ingresar. Mientras tanto, vamos a leer tu diagnóstico y coordinar la primera entrevista. Definamos el QUÉ juntos."}
          </p>
          <a
            href={loggedInEmpresa ? "/cuenta" : "/"}
          ...
          >
            {loggedInEmpresa ? "Volver a mi cuenta" : "Volver al inicio"}
          </a>
        </Container>
      </main>
    );
    ...
    <main className="min-h-screen bg-paper">
      ...
      <Container className="max-w-3xl py-16">
        <form onSubmit={submit} className="space-y-12">
          {/* Empresa (sólo si todavía no tiene cuenta) */}
          {!loggedInEmpresa && (
          ...
          <fieldset className="space-y-6">
            <legend className="mb-2 text-sm font-medium uppercase tracking-[0.12em] text-ink/50">
              {loggedInEmpresa ? "Tu contexto" : "02 · Tu contexto"}
            </legend>
          ...
          {/* Cuenta (sólo si todavía no tiene una) */}
          {!loggedInEmpresa && (
            <fieldset className="space-y-6">
              <legend className="mb-2 text-sm font-medium uppercase tracking-[0.12em] text-ink/50">
                03 · Creá tu cuenta
          ...
          <div className="flex flex-col items-start gap-4 border-t border-ink/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              type="submit"
              disabled={pending || (!loggedInEmpresa && (!passwordOk || !passwordsMatch))}
