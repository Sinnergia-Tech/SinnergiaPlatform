...
export function SocialLinksEditor({
  ...
  const save = () => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        setMsg({ ok: true, text: "Redes guardadas ✓" });
        router.refresh();
      } else {
        setMsg({ ok: false, text: res.error ?? "No se pudieron guardar las redes." });
      }
    });
  };
  ...
  return (
    <section className="border border-ink/10 bg-paper p-6">
      <h2 className="text-sm font-medium uppercase tracking-[0.12em] text-ink/50">Tus redes</h2>
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        {msg && (
          <span className={`text-sm ${msg.ok ? "text-ink/50" : "text-ink"}`}>
