...
export function ReactivateAccount({ suspendedByAdmin = false }: { suspendedByAdmin?: boolean }) {
  ...
  const reactivate = () =>
    startTransition(async () => {
      const res = await reactivateOwnAccountAction();
      if (res.ok) {
  ...
  // Suspensión hecha por el equipo: el usuario no la puede levantar solo.
  if (suspendedByAdmin) {
    return (
      <section className="border border-ink/15 bg-paper p-8 text-center">
        ...
        <h1 className="mt-6 text-2xl font-light">Tu cuenta está suspendida</h1>
        <p className="mx-auto mt-3 max-w-md text-sm text-ink/60">
          El equipo de Sinnergia suspendió tu cuenta. Si creés que es un error,
